...
export async function approve(
  ...
  const recordWorkFlowCol = this.db.collection<RecordWorkflow>(
    `workflow_${dsSerial}`
  );
  ...
  if (!wf) throw new HttpError(HttpCode.NOT_FOUND, 'workflowNotFound');
  ...
  wfStep.actions = wfStep.actions.map((a) =>
    a.user === issuer.serial ? { ...a, action: 'approve', date, message } : a
  );
  ...
  if (stepState === 'approve') {
    ...
    if (currStepIndex === wf.steps.length - 1) {
      ...
      if (ds.settings.history) {
        ...
        if (currState) dataRecordsModel.pushHistory(dsSerial, currState);
      }

      if (activeWf.trigger === 'delete')
        await mainCol.deleteOne({ serial: recSerial });
      else
        await mainCol.replaceOne({ serial: recSerial }, record, {
          upsert: true,
        });
