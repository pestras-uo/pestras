...
export class MapChartView implements OnChanges {
  ...
  ngOnChanges() {
    this.payload$ = this.getRegions(this.conf.options, this.data.fields, this.data.records)
      .pipe(
  ...
  private getRegions(options: MapDataVizOptions, fields: Field[], data: DataRecord[]) {
    const serials = new Set<string>();

    if (options.use_map?.region) {
      return options.use_map.only_children
        ? this.regionsState.selectMany(r => Serial.isChild(options.use_map?.region ?? '', r.serial, 1))
    ...
    for (const r of data)
      serials.add(r[regionField.name] as string ?? '');

    return this.regionsState.selectMany([...serials].filter(Boolean))
      .pipe(
