/* eslint-disable @typescript-eslint/no-explicit-any */
/* eslint-disable @typescript-eslint/no-unused-vars */
/* eslint-disable @angular-eslint/component-class-suffix */
/* eslint-disable @angular-eslint/component-selector */
import { Component, Input, Output, OnInit, EventEmitter } from '@angular/core';
import { NG_VALUE_ACCESSOR, NG_VALIDATORS, ControlValueAccessor, FormBuilder, Validators, AbstractControl, ValidationErrors } from '@angular/forms';
import { TypedEntity, SortStageOptions, IAggrPiplineStage, AggrStageTypes } from '@pestras/shared/data-model';
import { Serial } from '@pestras/shared/util';
import { untilDestroyed } from '@pestras/frontend/ui';

@Component({
  selector: 'app-sort-stage',
  templateUrl: './sort-stage.form.html',
  styles: [':host { display: block; }'],
  providers: [
    { provide: NG_VALUE_ACCESSOR, multi: true, useExisting: SortStageForm },
    { provide: NG_VALIDATORS, multi: true, useExisting: SortStageForm }
  ]
})
export class SortStageForm implements OnInit, ControlValueAccessor {
  private ud = untilDestroyed();

  protected readonly id = Serial.gen("STG");

  readonly form = this.fb.nonNullable.group({
    fields: this.fb.nonNullable.array([this.fb.nonNullable.group({
      field: ['', Validators.required],
      order: this.fb.nonNullable.control<1 | -1>(1, Validators.required)
    })])
  });

  readonly fieldsList = this.form.controls.fields;

  disabled = false;
  touched = false;

  @Input()
  fields: TypedEntity[] = [];
  @Input()
  fcClass = '';

  @Output()
  changes = new EventEmitter<IAggrPiplineStage<SortStageOptions>>();

  constructor(private readonly fb: FormBuilder) { }

  ngOnInit(): void {
    this.form.valueChanges
      .pipe(this.ud())
      .subscribe(() => {
        setTimeout(() => {
          const options: IAggrPiplineStage<SortStageOptions> = {
            type: AggrStageTypes.SORT,
            options: this.form.getRawValue()
          };
          
          this.onChange(options);
          !this.touched && this.onTouched();
          this.touched = true;
          this.changes.emit(options);
        })
      });
  }

  filterField(field: TypedEntity) {
    return !['serial', 'unknown', 'location'].includes(field.type);
  }

  mapField(field: TypedEntity) {
    return { name: field.name, value: field.name }
  }

  addSort(field?: { field: string; order: -1 | 1 }) {
    this.fieldsList.push(this.fb.nonNullable.group({
      field: [field?.field || '', Validators.required],
      order: this.fb.nonNullable.control<1 | -1>(field?.order || 1, Validators.required)
    }));
  }

  // ControlValueAccessor interface
  // --------------------------------------------------------------
  private onChange = (_: any) => {
    //
  };
  private onTouched = () => {
    //
  };

  writeValue(opt: IAggrPiplineStage<SortStageOptions>): void {
    this.fieldsList.clear();

    for (const field of (opt?.options?.fields || []))
      this.addSort(field);
  }

  registerOnChange(fn: any): void {
    this.onChange = fn;
  }

  registerOnTouched(fn: any): void {
    this.onTouched = fn;
  }

  setDisabledState(isDisabled: boolean): void {
    this.disabled = isDisabled;
  }

  validate(_: AbstractControl): ValidationErrors | null {
    return null;
  }
}
