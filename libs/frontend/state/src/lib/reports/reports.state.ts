...
export class ReportsState extends StatorGroupState<Report> {
  ...
  update(serial: string, title: string) {
    return this.service
      ...
      .pipe(
        tap((res) =>
          this._update(serial, { title, last_modified: new Date(res) })
        )
      );
  }
  ...
  addSlide(serial: string, data: ReportsApi.AddSlide.Body) {
    return this.service.addSlide({ serial }, data).pipe(
      tap((res) =>
        this._update(serial, (d) => {
          return {
            ...d,
  ...
  updateSlidesOrder(serial: string, slides: string[]) {
    return this.service
      .updateSlidesOrder({ serial }, { slides })
      .pipe(
        tap((res) =>
          this._update(serial, {
            slides_order: slides,
  ...
  updateSlide(
    serial: string,
    slide: string,
    data: ReportsApi.UpdateSlide.Body
  ) {
    return this.service.updateSlide({ serial, slide }, data).pipe(
      tap((res) =>
        this._update(serial, (d) => {
          return {
            ...d,
            ...
            slides: d.slides.map((t) => {
              return t.serial !== slide ? t : { ...t, ...data };
            }),
          };
        })
      )
  ...
  removeSlide(serial: string, slide: string) {
    return this.service.removeSlide({ serial, slide }).pipe(
      tap((res) =>
        this._update(serial, (d) => {
          return {
            ...d,
  ...
  addView(serial: string, data: ReportsApi.AddView.Body) {
    return this.service.addView({ serial }, data).pipe(
      tap((res) =>
        this._update(serial, (d) => {
          return {
            ...d,
            ...
            slides: d.slides.map((t) => {
              if (t.serial !== data.slide) return t;
  ...
  updateViewContent(serial: string, view: string, content: string) {
    return this.service.updateViewContent({ serial, view }, { content }).pipe(
      tap((res) =>
        this._update(serial, (d) => {
          return {
            ...d,
  ...
  updateViewImage(serial: string, view: string, image: File) {
    return this.service.updateViewImage({ serial, view }, { image }).pipe(
      tap((res) =>
        this._update(serial, (d) => {
          return {
            ...d,
  ...
  updateViewsOrder(serial: string, slide: string, views: string[]) {
    return this.service.updateViewsOrder({ serial, slide }, { views }).pipe(
      tap((res) =>
        this._update(serial, (d) => {
          return {
            ...d,
            last_modified: new Date(res),
            slides: d.slides.map((t) => {
              return t.serial !== slide ? t : { ...t, views_order: views };
            }),
          };
        })
      )
  ...
  updateView(serial: string, view: string, data: ReportsApi.UpdateView.Body) {
    return this.service.updateView({ serial, view }, data).pipe(
      tap((res) =>
        this._update(serial, (d) => {
          return {
            ...d,
  ...
  removeView(serial: string, view: string) {
    return this.service.removeView({ serial, view }).pipe(
      tap((res) =>
        this._update(serial, (d) => {
          return {
            ...d,
            ...
            slides: d.slides.map((s) => {
              if (!s.views_order.includes(view)) return s;
