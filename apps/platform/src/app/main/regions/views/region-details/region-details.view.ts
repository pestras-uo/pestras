...
import { PuiMapPolygonOptions } from '@pestras/frontend/ui';
...
export class RegionDetailsView implements OnChanges {
  ...
  ngOnChanges(): void {
    this.region$ = this.state.select(this.serial)
      .pipe(tap(r => {
        if (r) {
          this.zoom = r.zoom ?? 13;
          ...
          if (r.coords?.coordinates) {
            ...
            const coordinates = r.coords.coordinates.reduce((coords: { lat: number, lng: number }[], curr: { lat: number; lng: number; }[]) => {
