...
export class AddUserModal implements OnInit {
  ...
  readonly form = this.fb.nonNullable.group({
    username: [
      ...
      [
        Validators.required,
        Validators.pattern(/^[a-zA-Z0-9_\-.]{4,64}$/),
    ...
    password: [
      ...
      [
        Validators.required,
        Validators.pattern(/^[a-zA-Z0-9!@#$%^&*-_=+.|<>:;'"()]{8,64}$/),
      ],
    ],
  ...
  // form data
  readonly orgs$ = this.orgs.data$.pipe(
    map((list) =>
      list.filter((org) =>
        Serial.isBranch(org.serial, this.session.get()?.orgunit ?? '', true)
      )
    ),
        ...
        if (org === this.session.get()?.serial) {
          if (this.session.get()?.is_super)
            ...
            this.form.controls.roles.setValue(
              this.form.controls.roles.value.filter((r) => r === Role.ADMIN)
            );
          }
        } else
          this.roles = [
