...
export class UpdatePasswordComponent {
  ...
  constructor(private state: UsersState, private toast: ToastService) {}

  update(c: Record<string, any>) {
    this.preloader = true;

    this.state
      .updatePassword(this.user.serial, this.form.getRawValue().password)
      .subscribe({
        next: () => {
        ...
        error: (e) => {
          ...
          this.toast.msg(c['errors'][e?.error] || c['errors'].password, {
            type: 'error',
          });
